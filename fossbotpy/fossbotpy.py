...
class Client:
	...
	def __init__(self, email="", password="", token="", proxy_host=None, proxy_port=None, user_agent="random", locale="en-US", build_num="request", base_url="https://dev.fosscord.com/api/v9/", log={"console":True, "file":False}):
		...
		self.websocketurl = imports.Other(self.s, self.fosscord, self.log).getGatewayUrl().json()['url']
...
##########################################################
	...
	def checkToken(self, token):
		...
		settingsTest = imports.User(self.fosscord, editedS, self.log).enableDevMode().status_code == 200
		infoTest = imports.User(self.fosscord, editedS, self.log).info().status_code == 204
		if settingsTest:
	...
	def createDM(self,recipients):
		return imports.Messages(self.fosscord,self.s,self.log).createDM(recipients)

	#delete channel/DM/DM group
	def deleteChannel(self, channelID):
		return imports.Messages(self.fosscord,self.s,self.log).deleteChannel(channelID)

	#remove from DM group
	def removeFromDmGroup(self, channelID, userID):
		return imports.Messages(self.fosscord,self.s,self.log).removeFromDmGroup(channelID, userID)

	#add to DM group
	def addToDmGroup(self, channelID, userID):
		return imports.Messages(self.fosscord,self.s,self.log).addToDmGroup(channelID, userID)

	#create DM group invite link
	def createDmGroupInvite(self, channelID, max_age_seconds=86400):
		return imports.Messages(self.fosscord,self.s,self.log).createDmGroupInvite(channelID, max_age_seconds)

	#change DM group name
	def setDmGroupName(self, channelID, name):
		return imports.Messages(self.fosscord,self.s,self.log).setDmGroupName(channelID, name)

	#change DM icon
	def setDmGroupIcon(self, channelID, imagePath):
		return imports.Messages(self.fosscord,self.s,self.log).setDmGroupIcon(channelID, imagePath)

	#get recent messages
	def getMessages(self,channelID,num=1,beforeDate=None,aroundMessage=None): # num <= 100, beforeDate is a snowflake
		return imports.Messages(self.fosscord,self.s,self.log).getMessages(channelID,num,beforeDate,aroundMessage)

	#get message by channel ID and message ID
	def getMessage(self, channelID, messageID):
		return imports.Messages(self.fosscord,self.s,self.log).getMessage(channelID, messageID)

	#greet with stickers
	def greet(self, channelID, sticker_ids=["749054660769218631"]):
		return imports.Messages(self.fosscord,self.s,self.log).greet(channelID, sticker_ids)
	...
	def sendMessage(self, channelID, message="", nonce="calculate", tts=False, embed=None, message_reference=None, allowed_mentions=None, sticker_ids=None):
		return imports.Messages(self.fosscord,self.s,self.log).sendMessage(channelID, message, nonce, tts, embed, message_reference, allowed_mentions, sticker_ids)
	...
	def sendFile(self,channelID,filelocation,isurl=False,message="", tts=False, message_reference=None, sticker_ids=None):
		return imports.Messages(self.fosscord,self.s,self.log).sendFile(channelID,filelocation,isurl,message, tts, message_reference, sticker_ids)

	#reply, with a message and/or file
	def reply(self, guildID, channelID, messageID, message="", nonce="calculate", tts=False, embed=None, allowed_mentions={"parse":["users","roles","everyone"],"replied_user":False}, sticker_ids=None, file=None, isurl=False):
		return imports.Messages(self.fosscord,self.s,self.log).reply(guildID, channelID, messageID, message, nonce, tts, embed, allowed_mentions, sticker_ids, file, isurl)
	...
	def searchMessages(self, guildID=None, channelID=None, authorID=None, authorType=None, mentionsUserID=None, has=None, linkHostname=None, embedProvider=None, embedType=None, attachmentExtension=None, attachmentFilename=None, mentionsEveryone=None, includeNsfw=None, afterDate=None, beforeDate=None, textSearch=None, afterNumResults=None, limit=None):
		return imports.Messages(self.fosscord,self.s,self.log).searchMessages(guildID, channelID, authorID, authorType, mentionsUserID, has, linkHostname, embedProvider, embedType, attachmentExtension, attachmentFilename, mentionsEveryone, includeNsfw, afterDate, beforeDate, textSearch, afterNumResults, limit)
	...
	def filterSearchResults(self,searchResponse):
		return imports.Messages(self.fosscord,self.s,self.log).filterSearchResults(searchResponse)

	#sends the typing action for 10 seconds (or technically until you change the page)
	def typingAction(self,channelID):
		return imports.Messages(self.fosscord,self.s,self.log).typingAction(channelID)

	#delete message
	def deleteMessage(self,channelID,messageID):
		return imports.Messages(self.fosscord,self.s,self.log).deleteMessage(channelID,messageID)

	#edit message
	def editMessage(self,channelID,messageID,newMessage):
		return imports.Messages(self.fosscord,self.s,self.log).editMessage(channelID, messageID, newMessage)

	#pin message
	def pinMessage(self,channelID,messageID):
		return imports.Messages(self.fosscord,self.s,self.log).pinMessage(channelID,messageID)

	#un-pin message
	def unPinMessage(self,channelID,messageID):
		return imports.Messages(self.fosscord,self.s,self.log).unPinMessage(channelID,messageID)

	#get pinned messages
	def getPins(self,channelID):
		return imports.Messages(self.fosscord,self.s,self.log).getPins(channelID)

	#add reaction
	def addReaction(self,channelID,messageID,emoji):
		return imports.Messages(self.fosscord,self.s,self.log).addReaction(channelID,messageID,emoji)

	#remove reaction
	def removeReaction(self,channelID,messageID,emoji):
		return imports.Messages(self.fosscord,self.s,self.log).removeReaction(channelID,messageID,emoji)

	#acknowledge message (mark message read)
	def ackMessage(self,channelID,messageID,ackToken=None):
		return imports.Messages(self.fosscord,self.s,self.log).ackMessage(channelID,messageID,ackToken)

	#unacknowledge message (mark message unread)
	def unAckMessage(self,channelID,messageID,numMentions=0):
		return imports.Messages(self.fosscord,self.s,self.log).unAckMessage(channelID,messageID,numMentions)

	def bulkAck(self, data):
		return imports.Messages(self.fosscord,self.s,self.log).bulkAck(data)

	def getTrendingGifs(self, provider="tenor", locale="en-US", media_format="mp4"):
		return imports.Messages(self.fosscord,self.s,self.log).getTrendingGifs(provider, locale, media_format)

	'''
	Stickers
	'''
	def getStickers(self, directoryID="758482250722574376", store_listings=False, locale="en-US"):
		return imports.Stickers(self.fosscord,self.s,self.log).getStickers(directoryID, store_listings, locale)

	def getStickerFile(self, stickerID, stickerAsset): #this is an animated png
		return imports.Stickers(self.fosscord,self.s,self.log).getStickerFile(stickerID, stickerAsset)

	def getStickerJson(self, stickerID, stickerAsset):
		return imports.Stickers(self.fosscord,self.s,self.log).getStickerJson(stickerID, stickerAsset)

	def getStickerPack(self, stickerPackID):
		return imports.Stickers(self.fosscord,self.s,self.log).getStickerPack(stickerPackID)
	...
	def getRelationships(self):
		return imports.User(self.fosscord,self.s,self.log).getRelationships()

	#get mutual friends
	def getMutualFriends(self, userID):
		return imports.User(self.fosscord,self.s,self.log).getMutualFriends(userID)

	#create outgoing friend request
	def requestFriend(self, user): #you can input a userID(snowflake) or a user discriminator
		return imports.User(self.fosscord,self.s,self.log).requestFriend(user)

	#accept incoming friend request
	def acceptFriend(self, userID, location="friends"):
		return imports.User(self.fosscord,self.s,self.log).acceptFriend(userID, location)

	#remove friend OR unblock user
	def removeRelationship(self, userID, location="context menu"):
		return imports.User(self.fosscord,self.s,self.log).removeRelationship(userID, location)

	#block user
	def blockUser(self, userID, location="context menu"):
		return imports.User(self.fosscord,self.s,self.log).blockUser(userID, location)

	'''
	Other user stuff
	'''
	def getProfile(self, userID, with_mutual_guilds=True):
		return imports.User(self.fosscord,self.s,self.log).getProfile(userID, with_mutual_guilds)
	...
	def getUserAffinities(self):
		return imports.User(self.fosscord,self.s,self.log).getUserAffinities()

	def getGuildAffinities(self):
		return imports.User(self.fosscord,self.s,self.log).getGuildAffinities()

	def getMentions(self, limit=25, roleMentions=True, everyoneMentions=True):
		return imports.User(self.fosscord,self.s,self.log).getMentions(limit, roleMentions, everyoneMentions)

	def removeMentionFromInbox(self, messageID):
		return imports.User(self.fosscord,self.s,self.log).removeMentionFromInbox(messageID)

	def getMyStickers(self):
		return imports.User(self.fosscord,self.s,self.log).getMyStickers()

	def getNotes(self, userID):
		return imports.User(self.fosscord,self.s,self.log).getNotes(userID)

	def setUserNote(self, userID, note):
		return imports.User(self.fosscord,self.s,self.log).setUserNote(userID, note)

	def getRTCregions(self):
		return imports.User(self.fosscord,self.s,self.log).getRTCregions()

	def getVoiceRegions(self):
		return imports.User(self.fosscord,self.s,self.log).getVoiceRegions()
	...
	def setAvatar(self,imagePath):
		return imports.User(self.fosscord,self.s,self.log).setAvatar(imagePath)

	#set profile color
	def setProfileColor(self, color=None):
		return imports.User(self.fosscord,self.s,self.log).setProfileColor(color)

	#set username
	def setUsername(self, username): #USER PASSWORD NEEDS TO BE SET BEFORE THIS IS RUN
		return imports.User(self.fosscord,self.s,self.log).setUsername(username, password=self.__user_password)

	#set email
	def setEmail(self, email): #USER PASSWORD NEEDS TO BE SET BEFORE THIS IS RUN
		return imports.User(self.fosscord,self.s,self.log).setEmail(email, password=self.__user_password)

	#set password
	def setPassword(self, new_password): #USER PASSWORD NEEDS TO BE SET BEFORE THIS IS RUN
		return imports.User(self.fosscord,self.s,self.log).setPassword(new_password, password=self.__user_password)

	#set discriminator
	def setDiscriminator(self, discriminator): #USER PASSWORD NEEDS TO BE SET BEFORE THIS IS RUN
		return imports.User(self.fosscord,self.s,self.log).setDiscriminator(discriminator, password=self.__user_password)

	#set about me
	def setAboutMe(self, bio):
		return imports.User(self.fosscord,self.s,self.log).setAboutMe(bio)
	...
	def setDMscanLvl(self, level=1): # 0<=level<=2
		return imports.User(self.fosscord,self.s,self.log).setDMscanLvl(level)

	def allowDMsFromServerMembers(self, allow=True, disallowedGuildIDs=None):
		return imports.User(self.fosscord,self.s,self.log).allowDMsFromServerMembers(allow, disallowedGuildIDs)

	def allowFriendRequestsFrom(self, types=["everyone", "mutual_friends", "mutual_guilds"]):
		return imports.User(self.fosscord,self.s,self.log).allowFriendRequestsFrom(types)

	def analyticsConsent(self, grant=[], revoke=[]):
		return imports.User(self.fosscord,self.s,self.log).analyticsConsent(grant, revoke)

	def allowScreenReaderTracking(self, allow=True):
		return imports.User(self.fosscord,self.s,self.log).allowScreenReaderTracking(allow)

	def requestMyData(self):
		return imports.User(self.fosscord,self.s,self.log).requestMyData()

	def getConnectedAccounts(self):
		return imports.User(self.fosscord,self.s,self.log).getConnectedAccounts()

	def getConnectionUrl(self, accountType):
		return imports.User(self.fosscord,self.s,self.log).getConnectionUrl(accountType)

	def enableConnectionDisplayOnProfile(self, accountType, accountUsername, enable=True):
		return imports.User(self.fosscord,self.s,self.log).enableConnectionDisplayOnProfile(accountType, accountUsername, enable)

	def enableConnectionDisplayOnStatus(self, accountType, accountUsername, enable=True):
		return imports.User(self.fosscord,self.s,self.log).enableConnectionDisplayOnStatus(accountType, accountUsername, enable)

	def removeConnection(self, accountType, accountUsername):
		return imports.User(self.fosscord,self.s,self.log).removeConnection(accountType, accountUsername)

	def getBillingHistory(self, limit=20):
		return imports.User(self.fosscord,self.s,self.log).getBillingHistory(limit)

	def getPaymentSources(self):
		return imports.User(self.fosscord,self.s,self.log).getPaymentSources()

	def getBillingSubscriptions(self):
		return imports.User(self.fosscord,self.s,self.log).getBillingSubscriptions()

	def getStripeClientSecret(self):
		return imports.User(self.fosscord,self.s,self.log).getStripeClientSecret()

	def setTheme(self, theme): #"light" or "dark"
		return imports.User(self.fosscord,self.s,self.log).setTheme(theme)

	def setMessageDisplay(self, CozyOrCompact): #"cozy" or "compact"
		return imports.User(self.fosscord,self.s,self.log).setMessageDisplay(CozyOrCompact)

	def enableGifAutoPlay(self, enable=True): #boolean, default=True
		return imports.User(self.fosscord,self.s,self.log).enableGifAutoPlay(enable)

	def enableAnimatedEmoji(self, enable=True): #boolean, default=True
		return imports.User(self.fosscord,self.s,self.log).enableAnimatedEmoji(enable)

	def setStickerAnimation(self, setting): #string, default="always"
		return imports.User(self.fosscord,self.s,self.log).setStickerAnimation(setting)

	def enableTTS(self, enable=True): #boolean, default=True
		return imports.User(self.fosscord,self.s,self.log).enableTTS(enable)

	def enableLinkedImageDisplay(self, enable=True): #boolean, default=True
		return imports.User(self.fosscord,self.s,self.log).enableLinkedImageDisplay(enable)

	def enableImageDisplay(self, enable=True): #boolean, default=True
		return imports.User(self.fosscord,self.s,self.log).enableImageDisplay(enable)

	def enableLinkPreview(self, enable=True): #boolean, default=True
		return imports.User(self.fosscord,self.s,self.log).enableLinkPreview(enable)

	def enableReactionRendering(self, enable=True): #boolean, default=True
		return imports.User(self.fosscord,self.s,self.log).enableReactionRendering(enable)

	def enableEmoticonConversion(self, enable=True): #boolean, default=True
		return imports.User(self.fosscord,self.s,self.log).enableEmoticonConversion(enable)

	def setAFKtimeout(self, timeout_seconds):
		return imports.User(self.fosscord,self.s,self.log).setAFKtimeout(timeout_seconds)

	def setLocale(self, locale):
		response = imports.User(self.fosscord,self.s,self.log).setLocale(locale)
		self.locale = locale
	...
	def enableDevMode(self, enable=True): #boolean
		return imports.User(self.fosscord,self.s,self.log).enableDevMode(enable)

	def activateApplicationTestMode(self, applicationID):
		return imports.User(self.fosscord,self.s,self.log).activateApplicationTestMode(applicationID)

	def getApplicationData(self, applicationID, with_guild=False):
		return imports.User(self.fosscord,self.s,self.log).getApplicationData(applicationID, with_guild)

	def enableActivityDisplay(self, enable=True):
		return imports.User(self.fosscord,self.s,self.log).enableActivityDisplay(enable)

	def setHypesquad(self, house):
		return imports.User(self.fosscord,self.s,self.log).setHypesquad(house)

	def leaveHypesquad(self):
		return imports.User(self.fosscord,self.s,self.log).leaveHypesquad()

	def getBuildOverrides(self):
		return imports.User(self.fosscord,self.s,self.log).getBuildOverrides()

	def enableSourceMaps(self, enable=True):
		return imports.User(self.fosscord,self.s,self.log).enableSourceMaps()

	def suppressEveryonePings(self, guildID, suppress=True):
		return imports.User(self.fosscord,self.s,self.log).suppressEveryonePings(guildID, suppress)

	def suppressRoleMentions(self, guildID, suppress=True):
		return imports.User(self.fosscord,self.s,self.log).suppressRoleMentions(guildID, suppress)

	def enableMobilePushNotifications(self, guildID, enable=True):
		return imports.User(self.fosscord,self.s,self.log).enableMobilePushNotifications(guildID, enable)

	def setChannelNotificationOverrides(self, guildID, overrides):
		return imports.User(self.fosscord,self.s,self.log).setChannelNotificationOverrides(guildID, overrides)

	def setMessageNotifications(self, guildID, notifications):
		return imports.User(self.fosscord,self.s,self.log).setMessageNotifications(guildID, notifications)

	def muteGuild(self, guildID, mute=True, duration=None):
		return imports.User(self.fosscord,self.s,self.log).muteGuild(guildID, mute, duration)

	def muteDM(self, DMID, mute=True, duration=None):
		return imports.User(self.fosscord,self.s,self.log).muteDM(DMID, mute, duration)

	def setThreadNotifications(self, threadID, notifications):
		return imports.User(self.fosscord,self.s,self.log).setThreadNotifications(threadID, notifications)

	def getReportMenu(self):
		return imports.User(self.fosscord,self.s,self.log).getReportMenu()

	def reportSpam(self, channelID, messageID, reportType="first_dm", guildID=None, version="1.0", variant="1", language="en"):
		return imports.User(self.fosscord,self.s,self.log).reportSpam(channelID, messageID, reportType, guildID, version, variant, language)

	def getHandoffToken(self, key):
		return imports.User(self.fosscord,self.s,self.log).getHandoffToken(key)

	def inviteToCall(self, channelID, userIDs=None):
		return imports.User(self.fosscord,self.s,self.log).inviteToCall(channelID, userIDs)

	def declineCall(self, channelID):
		return imports.User(self.fosscord,self.s,self.log).declineCall(channelID)
	...
	def getInfoFromInviteCode(self,inviteCode, with_counts=True, with_expiration=True, fromJoinGuildNav=False):
		return imports.Guild(self.fosscord,self.s,self.log).getInfoFromInviteCode(inviteCode, with_counts, with_expiration, fromJoinGuildNav)

	#join guild with invite code
	def joinGuild(self, inviteCode, location="accept invite page", wait=0):
		return imports.Guild(self.fosscord,self.s,self.log).joinGuild(inviteCode, location, wait)

	#preview/lurk-join guild. Only applies to current (gateway) session
	def previewGuild(self, guildID, sessionID=None):
		return imports.Guild(self.fosscord,self.s,self.log).previewGuild(guildID, sessionID)

	#leave guild
	def leaveGuild(self, guildID, lurking=False):
		return imports.Guild(self.fosscord,self.s,self.log).leaveGuild(guildID, lurking)

	#create invite
	def createInvite(self, channelID, max_age_seconds=False, max_uses=False, grantTempMembership=False, checkInvite="", targetType=""):
		return imports.Guild(self.fosscord,self.s,self.log).createInvite(channelID, max_age_seconds, max_uses, grantTempMembership, checkInvite, targetType)

	def deleteInvite(self, inviteCode):
		return imports.Guild(self.fosscord,self.s,self.log).deleteInvite(inviteCode)

	def getGuildInvites(self, guildID):
		return imports.Guild(self.fosscord,self.s,self.log).getGuildInvites(guildID)

	def getChannelInvites(self, channelID):
		return imports.Guild(self.fosscord,self.s,self.log).getChannelInvites(channelID)

	#get all guilds (this is used by the client when going to the developers portal)
	def getGuilds(self, with_counts=True):
		return imports.Guild(self.fosscord,self.s,self.log).getGuilds(with_counts)
	...
	def getGuildChannels(self, guildID):
		return imports.Guild(self.fosscord,self.s,self.log).getGuildChannels(guildID)

	#get discoverable guilds
	def getDiscoverableGuilds(self, offset=0, limit=24):
		return imports.Guild(self.fosscord,self.s,self.log).getDiscoverableGuilds(offset, limit)

	def getGuildRegions(self, guildID):
		return imports.Guild(self.fosscord,self.s,self.log).getGuildRegions(guildID)

	#create a guild
	def createGuild(self, name, icon=None, channels=[], systemChannelID=None, template="2TffvPucqHkN"):
		return imports.Guild(self.fosscord,self.s,self.log).createGuild(name, icon, channels, systemChannelID, template)

	#delete a guild
	def deleteGuild(self, guildID):
		return imports.Guild(self.fosscord,self.s,self.log).deleteGuild(guildID)

	#kick a user
	def kick(self,guildID,userID,reason=""):
		return imports.Guild(self.fosscord,self.s,self.log).kick(guildID,userID,reason)

	#ban a user
	def ban(self,guildID,userID,deleteMessagesDays=0,reason=""):
		return imports.Guild(self.fosscord,self.s,self.log).ban(guildID,userID,deleteMessagesDays,reason)

	#unban a user
	def revokeBan(self, guildID, userID):
		return imports.Guild(self.fosscord,self.s,self.log).revokeBan(guildID, userID)

	#get number of members in each role
	def getRoleMemberCounts(self, guildID):
		return imports.Guild(self.fosscord,self.s,self.log).getRoleMemberCounts(guildID)

	#get integrations (includes applications aka bots)
	def getGuildIntegrations(self, guildID, include_applications=True):
		return imports.Guild(self.fosscord,self.s,self.log).getGuildIntegrations(guildID, include_applications)

	#get guild templates
	def getGuildTemplates(self, guildID):
		return imports.Guild(self.fosscord,self.s,self.log).getGuildTemplates(guildID)

	#get role member ids
	def getRoleMemberIDs(self, guildID, roleID):
		return imports.Guild(self.fosscord,self.s,self.log).getRoleMemberIDs(guildID, roleID)

	#add members to role (add a role to multiple members at the same time)
	def addMembersToRole(self, guildID, roleID, memberIDs):
		return imports.Guild(self.fosscord,self.s,self.log).addMembersToRole(guildID, roleID, memberIDs)

	#set roles of a member
	def setMemberRoles(self, guildID, memberID, roleIDs):
		return imports.Guild(self.fosscord,self.s,self.log).setMemberRoles(guildID, memberID, roleIDs)

	def getMemberVerificationData(self, guildID, with_guild=False, invite_code=None):
		return imports.Guild(self.fosscord,self.s,self.log).getMemberVerificationData(guildID, with_guild, invite_code)

	def agreeGuildRules(self, guildID, form_fields, version="2021-01-05T01:44:32.163000+00:00"):
		return imports.Guild(self.fosscord,self.s,self.log).agreeGuildRules(guildID, form_fields, version)

	def createThread(self, channelID, name, messageID=None, public=True, archiveAfter='24 hours'):
		return imports.Guild(self.fosscord,self.s,self.log).createThread(channelID, name, messageID, public, archiveAfter)

	def leaveThread(self, threadID, location="Sidebar Overflow"):
		return imports.Guild(self.fosscord,self.s,self.log).leaveThread(threadID, location)

	def joinThread(self, threadID, location="Banner"):
		return imports.Guild(self.fosscord,self.s,self.log).joinThread(threadID, location)

	def archiveThread(self, threadID, lock=True):
		return imports.Guild(self.fosscord,self.s,self.log).archiveThread(threadID, lock)

	def unarchiveThread(self, threadID, lock=False):
		return imports.Guild(self.fosscord,self.s,self.log).unarchiveThread(threadID, lock)

	def lookupSchool(self, email, allowMultipleGuilds=True, useVerificationCode=True):
		return imports.Guild(self.fosscord,self.s,self.log).lookupSchool(email, allowMultipleGuilds, useVerificationCode)

	def schoolHubWaitlistSignup(self, email, school):
		return imports.Guild(self.fosscord,self.s,self.log).schoolHubWaitlistSignup(email, school)

	def schoolHubSignup(self, email, hubID):
		return imports.Guild(self.fosscord,self.s,self.log).schoolHubSignup(email, hubID)

	def verifySchoolHubSignup(self, hubID, email, code):
		return imports.Guild(self.fosscord,self.s,self.log).verifySchoolHubSignup(hubID, email, code)

	def getSchoolHubGuilds(self, hubID):
		return imports.Guild(self.fosscord,self.s,self.log).getSchoolHubGuilds(hubID)

	def getSchoolHubDirectoryCounts(self, hubID):
		return imports.Guild(self.fosscord,self.s,self.log).getSchoolHubDirectoryCounts(hubID)

	def joinGuildFromSchoolHub(self, hubID, guildID):
		return imports.Guild(self.fosscord,self.s,self.log).joinGuildFromSchoolHub(hubID, guildID)

	def searchSchoolHub(self, hubID, query):
		return imports.Guild(self.fosscord,self.s,self.log).searchSchoolHub(hubID, query)

	def getMySchoolHubGuilds(self, hubID):
		return imports.Guild(self.fosscord,self.s,self.log).getMySchoolHubGuilds(hubID)

	def setSchoolHubGuildDetails(self, hubID, guildID, description, directoryID):
		return imports.Guild(self.fosscord,self.s,self.log).setSchoolHubGuildDetails(hubID, guildID, description, directoryID)

	def getLiveStages(self, extra=False):
		return imports.Guild(self.fosscord,self.s,self.log).getLiveStages(extra)

	def getChannel(self, channelID):
		return imports.Guild(self.fosscord,self.s,self.log).getChannel(channelID)

	def getGuildActivitiesConfig(self, guildID):
		return imports.Guild(self.fosscord,self.s,self.log).getGuildActivitiesConfig(guildID)
