...
class User(object):
	...
	def getMutualFriends(self, userID):
		url = self.fosscord+"users/"+userID+"/relationships"
		return Wrapper.sendRequest(self.s, 'get', url, log=self.log)
	...
	def acceptFriend(self, userID, location):
		url = self.fosscord+"users/@me/relationships/"+userID
		body = {}
	...
	def getProfile(self, userID, with_mutual_guilds):
		url = self.fosscord+"users/"+userID+"/profile"
	...
	def getMentions(self, limit, roleMentions, everyoneMentions):
		...
		url = self.fosscord+"users/@me/mentions?limit="+str(limit)+"&roles="+roleMentions+"&everyone="+everyoneMentions
		return Wrapper.sendRequest(self.s, 'get', url, log=self.log)

	def removeMentionFromInbox(self, messageID):
		url = self.fosscord+"users/@me/mentions/"+messageID
	...
	def getNotes(self, userID):
		url = self.fosscord+"users/@me/notes/"+userID
		return Wrapper.sendRequest(self.s, 'get', url, log=self.log)
	...
	def allowScreenReaderTracking(self, allow): #more fosscord tracking stuff
		url = self.fosscord+"users/@me/settings"
	...
	def getConnectionUrl(self, accountType):
		url = self.fosscord+"connections/"+accountType+"/authorize"
		return Wrapper.sendRequest(self.s, 'get', url, log=self.log)

	def enableConnectionDisplayOnProfile(self, accountType, accountUsername, enable):
		url = self.fosscord+"users/@me/connections/"+accountType+"/"+accountUsername
	...
	def getStripeClientSecret(self): #for adding new payment methods. Stripe api wraps are not included because discum is just a fosscord api wrapper.
	...
	def activateApplicationTestMode(self, applicationID):
		url = self.fosscord+"applications/"+applicationID+"/skus"
		return Wrapper.sendRequest(self.s, 'get', url, log=self.log)

	def getApplicationData(self, applicationID, with_guild):
		url = self.fosscord+"applications/"+applicationID+"/public?with_guild="+str(with_guild).lower()
		return Wrapper.sendRequest(self.s, 'get', url, log=self.log)
	...
	def suppressEveryonePings(self, guildID, suppress):
		url = self.fosscord+"users/@me/guilds/"+str(guildID)+"/settings"
	...
	def setThreadNotifications(self, threadID, notifications):
		url = self.fosscord+"channels/"+threadID+"/thread-members/@me/settings"
	...
	def reportSpam(self, channelID, messageID, reportType, guildID, version, variant, language):
		url = self.fosscord+'reporting/'+reportType
		body = {
	...
	def inviteToCall(self, channelID, userIDs):
		url = self.fosscord+'channels/'+channelID+'/call/ring'
		body = {"recipients": userIDs}
		return Wrapper.sendRequest(self.s, 'post', url, body, log=self.log)

	def declineCall(self, channelID):
		url = self.fosscord+'channels/'+channelID+'/call/stop-ringing'
		body = {}
		return Wrapper.sendRequest(self.s, 'post', url, body, log=self.log)
